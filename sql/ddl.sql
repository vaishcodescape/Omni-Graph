...
CREATE TABLE access_policies (
    ...
    resource_type     VARCHAR(50) NOT NULL CHECK (resource_type IN (
                          'document', 'entity', 'concept', 'audit_log'
                      )),
    sensitivity_level VARCHAR(30) NOT NULL CHECK (sensitivity_level IN (
...
CREATE TABLE concept_hierarchy (
    ...
    relationship_type VARCHAR(50) NOT NULL DEFAULT 'is_parent_of'
                                  CHECK (relationship_type IN (
                                      'is_parent_of', 'is_specialization_of', 'is_prerequisite_of'
                                  )),
    created_at        TIMESTAMP   NOT NULL DEFAULT CURRENT_TIMESTAMP,
